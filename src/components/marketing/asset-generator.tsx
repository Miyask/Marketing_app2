    ...
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
      <Card className="xl:col-span-4 glass-card border-none shadow-xl h-fit">
        <CardHeader>
          ...
          <CardDescription className="text-muted-foreground/60">Define los valores de marca para la IA.</CardDescription>
        </CardHeader>
      ...
      <div className="xl:col-span-8 space-y-6">
        {!output && !loading && (
          <div className="border border-dashed border-white/10 rounded-3xl h-[600px] flex flex-col items-center justify-center text-center p-12 bg-white/5">
            ...
            <h3 className="text-2xl font-headline font-bold text-white mb-2">Creatividad Instantánea</h3>
        ...
        {loading && (
          <div className="h-[600px] flex flex-col items-center justify-center text-center space-y-4">
            ...
            <h3 className="text-xl font-headline font-bold text-white animate-pulse">Destilando Esencia de Marca...</h3>
          </div>
        )}
        ...
        {output && (
          <div className="animate-fade-in space-y-6 pb-12">
            <Tabs defaultValue="overview" className="w-full">
              <TabsList className="bg-white/5 border-white/10 p-1 mb-8 rounded-2xl h-14">
                <TabsTrigger value="overview" className="rounded-xl px-8 font-bold uppercase text-[10px]">Resumen Creativo</TabsTrigger>
                <TabsTrigger value="logo" className="rounded-xl px-8 font-bold uppercase text-[10px]">Concepto Logo</TabsTrigger>
                <TabsTrigger value="brochure" className="rounded-xl px-8 font-bold uppercase text-[10px]">Estructura Folleto</TabsTrigger>
              </TabsList>
                  ...
                  <Card className="glass-card border-none p-6">
                    ...
                    <div className="space-y-3">
                      {output.slogans.map((slogan, i) => (
                        <div key={i} className="group p-4 rounded-xl bg-white/5 border border-white/5 hover:border-primary/30 transition-all flex items-center justify-between">
                          <p className="text-sm font-medium italic text-white/90">"{slogan}"</p>
                          <Button variant="ghost" size="icon" onClick={() => copyToClipboard(slogan)} className="opacity-0 group-hover:opacity-100 transition-opacity"><Copy className="w-3 h-3 text-white/40" /></Button>
                        </div>
                      ))}
                    </div>
                  </Card>
                  ...
                  <Card className="glass-card border-none p-6">
                    ...
                    <div className="space-y-3">
                      {output.marketingIdeas.slice(0, 3).map((idea, i) => (
                        <div key={i} className="p-4 rounded-xl border border-white/5 bg-white/5 text-xs text-muted-foreground leading-relaxed">
              ...
              <TabsContent value="logo">
                <Card className="glass-card border-none p-8">
                  <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                      <h3 className="text-2xl font-headline font-bold text-white">Dirección de Arte</h3>
                      <p className="text-sm text-muted-foreground">Concepto visual y paleta sugerida por IA.</p>
                    </div>
                  </div>
                  <div className="bg-white/5 border border-white/10 rounded-2xl p-6 leading-relaxed text-lg text-white/80 italic">
                    {output.logoConcept}
                  </div>
                </Card>
              </TabsContent>
              ...
              <TabsContent value="brochure">
                <Card className="glass-card border-none p-8">
                  <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                      <h3 className="text-2xl font-headline font-bold text-white">Mockup de Contenidos</h3>
                      <p className="text-sm text-muted-foreground">Estructura de secciones para tus materiales impresos o PDF.</p>
                    </div>
                  </div>
                  <div className="bg-white/5 border border-white/10 rounded-2xl p-8 whitespace-pre-wrap font-mono text-sm text-muted-foreground">
